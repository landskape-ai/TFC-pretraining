...
def scatter_numpy(self, dim, index, src):
    ...
    if (index >= self.shape[dim]).any() or (index < 0).any():
        raise IndexError(
            "The values of index must be between 0 and (self.shape[dim] -1)"
        )
    ...
    if not np.isscalar(src):
        if index.shape[dim] > src.shape[dim]:
            raise IndexError(
                "Dimension " + str(dim) + "of index can not be bigger than that of src "
            )
        ...
        src_idx.pop(dim)
        src_idx.insert(
            dim, np.repeat(np.arange(index.shape[dim]), np.prod(idx_xsection_shape))
        )
...
for dataset_name, n_classes in zip(dataset_lst, n_classes_lst):
    ...
    np.save(
        os.path.join(savepath, "train_x.npy"), train_dict["samples"].transpose(1, 2)
    )
    ...
    scatter_numpy(
        train_y, 1, np.expand_dims(train_dict["labels"].numpy().astype(int), axis=1), 1
    )
    scatter_numpy(
        val_y, 1, np.expand_dims(val_dict["labels"].numpy().astype(int), axis=1), 1
    )
    scatter_numpy(
        test_y, 1, np.expand_dims(test_dict["labels"].numpy().astype(int), axis=1), 1
    )
