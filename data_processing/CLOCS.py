...
for alias, dirname, trial, modality in zip(
    alias_lst, dirname_lst, trial_lst, modality_lst
):
    ...
    output_dict[modality][fraction]["train"][term] = np.expand_dims(
        train_dict["labels"], axis=1
    )
    output_dict[modality][fraction]["test"][term] = np.expand_dims(
        test_dict["labels"], axis=1
    )
    output_dict[modality][fraction]["val"][term] = np.expand_dims(
        val_dict["labels"], axis=1
    )

    ctr = 0
    pid_dict[modality][fraction]["train"][term] = np.expand_dims(
        np.arange(ctr, ctr + len(train_dict["labels"])), axis=1
    )
    ctr += len(train_dict["labels"])
    pid_dict[modality][fraction]["test"][term] = np.expand_dims(
        np.arange(ctr, ctr + len(test_dict["labels"])), axis=1
    )
    ctr += len(test_dict["labels"])
    pid_dict[modality][fraction]["val"][term] = np.expand_dims(
        np.arange(ctr, ctr + len(val_dict["labels"])), axis=1
    )
