...
def random_masking(x, mask_ratio):
    ...
    x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, hidden_dim))
...
class Time_Encoder(nn.Module):
    def __init__(self, configs):
        ...
        self.pos_embedder_t = nn.Embedding(
            configs.TSlength_aligned, configs.hidden_dim
        )
    ...
    def forward(self, x_in_t):
        ...
        pos_ids = torch.arange(x_in_t.size(1), dtype=torch.long, device=x_in_t.device).unsqueeze(0)
        ...
        return h_time, z_time, mask_t, ids_restore_t
...
class Freq_Encoder(nn.Module):
    def __init__(self, configs):
        ...
        self.pos_embedder_f = nn.Embedding(
            configs.TSlength_aligned, configs.hidden_dim
        )
    ...
    def forward(self, x_in_f):
        ...
        pos_ids = torch.arange(x_in_f.size(1), dtype=torch.long, device=x_in_f.device).unsqueeze(0)
        ...
        return h_freq, z_freq, mask_f, ids_restore_f
...
class Time_Decoder(nn.Module):
    def __init__(self, configs):
        ...
        self.transformer_decoder_t = TransformerEncoder(decoder_layers_t, configs.num_layers)
        self.head_t = nn.Linear(configs.hidden_dim, configs.input_channels)

    def forward(self, h_time):
        """ Add positional embedding """
        pos_ids = torch.arange(h_time.size(1), dtype=torch.long, device=h_time.device).unsqueeze(0)
        pos_embeds = self.pos_embedder_t(pos_ids)
...
class Freq_Decoder(nn.Module):
    def __init__(self, configs):
        ...
        self.transformer_decoder_f = TransformerEncoder(decoder_layers_f, configs.num_layers)
        self.head_f = nn.Linear(configs.hidden_dim, configs.input_channels)


    def forward(self, h_freq):
        """ Add positional embedding """
        pos_ids = torch.arange(h_freq.size(1), dtype=torch.long, device=h_freq.device).unsqueeze(0)
        pos_embeds = self.pos_embedder_f(pos_ids)
...
class TFC(nn.Module):
    ...
    def forward(self, x_in_t, x_in_f):
        """Use Transformer"""
        h_time, z_time, mask_t, ids_restore_t = self.time_encoder(x_in_t)
        h_freq, z_freq, mask_f, ids_restore_f = self.freq_encoder(x_in_f)
        ...
        mask_tokens = torch.zeros(1, 1, self.configs.hidden_dim).repeat(h_time.shape[0], ids_restore_t.shape[1] + 1 - h_time.shape[1], 1)
        h_time_ = torch.cat([h_time, mask_tokens], dim=1)
        h_time = torch.gather(h_time_, dim=1, index=ids_restore_t.unsqueeze(-1).repeat(1, 1, h_time.shape[2]))
        
        mask_tokens = torch.zeros(1, 1, self.configs.hidden_dim).repeat(h_freq.shape[0], ids_restore_f.shape[1] + 1 - h_freq.shape[1], 1)
        h_freq_ = torch.cat([h_freq, mask_tokens], dim=1)
        h_freq = torch.gather(h_freq_, dim=1, index=ids_restore_f.unsqueeze(-1).repeat(1, 1, h_freq.shape[2]))
        
        """ Reconstruct the masked input sequence """
        h_time = self.time_decoder(h_time)
        h_freq = self.freq_decoder(h_freq)
        
        return h_time, h_freq, z_time, z_freq, mask_t, mask_f
