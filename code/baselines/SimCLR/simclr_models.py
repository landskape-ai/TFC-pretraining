...
def create_base_model(input_shape, model_name="base_model"):
    ...
    x = tf.keras.layers.Conv1D(
        32, 24, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(l=1e-4)
    )(x)
...
def create_linear_model_from_base_model(base_model, output_shape, intermediate_layer=7):
    ...
    x = tf.keras.layers.Dense(
        output_shape, kernel_initializer=tf.random_normal_initializer(stddev=0.01)
    )(x)
    outputs = tf.keras.layers.Softmax()(x)

    model = tf.keras.Model(
        inputs=inputs, outputs=outputs, name=base_model.name + "linear"
    )
