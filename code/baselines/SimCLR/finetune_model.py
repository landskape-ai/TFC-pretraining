...
data_folder = "pFD_B"
np_train = (
    np.load(os.path.join(data_folder, "train_x.npy")),
    np.load(os.path.join(data_folder, "train_y.npy")),
)
np_val = (
    np.load(os.path.join(data_folder, "val_x.npy")),
    np.load(os.path.join(data_folder, "val_y.npy")),
)
np_test = (
    np.load(os.path.join(data_folder, "test_x.npy")),
...
pretrained_model = tf.keras.models.load_model(
    os.path.join(working_directory, f"{seed}_simclr.hdf5")
)

linear_evaluation_model = simclr_models.create_linear_model_from_base_model(
    pretrained_model, output_shape=output_shape
)

linear_eval_best_model_file_name = (
    f"{working_directory}{start_time_str}_finetuning.hdf5"
)
...
print(
    simclr_utitlities.evaluate_model_simple(
        linear_eval_best_model.predict(np_test[0]), np_test[1], return_dict=True
    )
)
print("Model in last epoch")
print(
    simclr_utitlities.evaluate_model_simple(
        linear_evaluation_model.predict(np_test[0]), np_test[1], return_dict=True
    )
)
