...
def generate_slicing_transform_function(
...
def get_NT_Xent_loss_gradients(
    model,
    samples_transform_1,
    samples_transform_2,
    normalize=True,
...
def simclr_train_model(
    ...
    for epoch in range(epochs):
        ...
        batched_dataset = data_pre_processing.get_batched_dataset_generator(
            shuffled_dataset, batch_size
        )
        ...
        for data_batch in batched_dataset:
            ...
            if is_trasnform_function_vectorized:
                ...
                transform_1 = np.array(
                    [transformation_function(data) for data in data_batch]
                )
            ...
            # Forward propagation
            loss, gradients = get_NT_Xent_loss_gradients(
                model,
                transform_1,
                transform_2,
                normalize=True,
...
def evaluate_model_simple(pred, truth, is_one_hot=True, return_dict=True):
    ...
    test_precision = sklearn.metrics.precision_score(
        truth_argmax, pred_argmax, average="macro"
    )
    ...
    test_f1_weighted = sklearn.metrics.f1_score(
        truth_argmax, pred_argmax, average="weighted"
    )
