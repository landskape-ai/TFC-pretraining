...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="The hidden dimension (defaults to 64)",
    )
    parser.add_argument(
        "--max-train-length",
        type=int,
    ...
    parser.add_argument(
        "--train",
        action="store_true",
        help="Whether to perform training, if not, will load model pretrained on the second dataset",
    )
    parser.add_argument(
        "--irregular",
        type=float,
    ...
    # the data are read directly from preprocessed numpy files, available upon request
        ...
        task_type = "classification"
        train_data, train_labels, test_data, test_labels = datautils.load_baseline(
            args.dataset
        )
