...
#%%
def load_initial_model_contrastive(
    cnn_network,
    phases,
    save_path_dir,
    saved_weights,
        ...
        if "train1" in phases:
            ...
            if trial_to_run in ["Linear", "Fine-Tuning"]:
                model.load_state_dict(
                    torch.load(os.path.join(save_path_dir, saved_weights))
                )
    ...
    optimizer = optim.Adam(
        list(model.parameters()), lr=held_out_lr, weight_decay=0
    )  # shouldn't load this again - will lose running average of gradients and so forth
