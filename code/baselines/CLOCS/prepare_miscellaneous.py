...
def flatten_arrays(
    outputs_list, labels_list, modality_list, indices_list, task_names_list, pids_list
):
...
def obtain_contrastive_loss(latent_embeddings, pids, trial):
    ...
    for combination in view_combinations:
        ...
        if trial == "CMC":
            ...
            """ Obtain Loss Terms(s) """
            loss_term1 = -torch.mean(
                torch.log(diagonals / torch.sum(sim_matrix_exp, 1))
            )
            loss_term2 = -torch.mean(
                torch.log(diagonals / torch.sum(sim_matrix_exp, 0))
            )
            ...
            self_sim_matrix_off_diagonals1 = torch.triu(
                self_sim_matrix_exp1, 1
            ) + torch.tril(self_sim_matrix_exp1, -1)
            ...
            self_sim_matrix_off_diagonals2 = torch.triu(
                self_sim_matrix_exp2, 1
            ) + torch.tril(self_sim_matrix_exp2, -1)

            denominator_loss1 = torch.sum(sim_matrix_exp, 1) + torch.sum(
                self_sim_matrix_off_diagonals1, 1
            )
            denominator_loss2 = torch.sum(sim_matrix_exp, 0) + torch.sum(
                self_sim_matrix_off_diagonals2, 0
            )
            ...
            "CMLC",
            "CMSMLC",
        ]:  # ours #CMSMLC = positive examples are same instance and same patient
...
def calculate_acc(outputs_list, labels_list, save_path_dir):
    if (
        "physionet2020" in save_path_dir or "ptbxl" in save_path_dir
    ):  # multilabel scenario
        ...
        labels_list = [
            np.where(multi_hot_vector)[0] for multi_hot_vector in labels_list
        ]
        """ What Proportion of Labels Did you Get Right """
        acc = np.array(
            [
                np.isin(preds, labels).sum()
                for preds, labels in zip(preds_list, labels_list)
            ]
        ).sum() / (len(np.concatenate(preds_list)))
...
def save_config_weights(
    ...
    trial,
    downstream_dataset,
):  # which is actually second_dataset
...
def modify_dataset_order_for_multi_task_learning(
    dataset, modalities, leads, class_pairs, fractions
):
    dataset = [
        dataset
    ]  # outside of if statement because dataset is original during each iteration
    if not isinstance(
        fractions, list
    ):  # it is already in list format, therefore no need for extra list
    ...
    if trial_to_run in ["Linear", "Fine-Tuning", "Random"]:
        ...
        if trial_to_run in ["Random"]:
            ...
            if second_dataset in ["chapman", "physionet2020"]:
                leads_path = "leads_%s" % str(
                    leads[0]
                )  # only these two datasets have multiple leads
            else:
                leads_path = ""
        ...
        save_path_dir = os.path.join(
            ...
            second_dataset,
            leads_path,
            labelled_fraction_path,
        )
        # print(save_path_dir)
        if "train" in phases:
            save_path_dir, seed = make_dir(
                save_path_dir,
                max_seed,
                task,
                trial_to_run,
            ...
            )  # do NOT change second_pass = True b/c this function is only ever used during second pass
        elif "test" in phases:
        ...
        if save_path_dir in ["do not train", "do not test"]:
            ...
            seed_index = np.where(["seed" in token for token in split_save_path_dir])[
                0
            ].item()
            load_path_dir = "/".join(
                split_save_path_dir[: seed_index + 1]
            )  # you want to exclude everything AFTER the seed path
    else:
        load_path_dir = save_path_dir
...
def make_saving_directory_contrastive(
    phases,
    dataset_name,
    trial_to_load,
    trial_to_run,
    ...
    if "train" in phases:
        save_path_dir, seed = make_dir(
            save_path_dir, max_seed, task, trial_to_run, evaluation=evaluation
        )
    elif "test" in phases:
        if "test_metrics_dict" in os.listdir(save_path_dir):
            save_path_dir = "do not test"
...
def make_dir(
    save_path_dir, max_seed, task, trial_to_run, second_pass=False, evaluation=False
):  # boolean allows you to overwrite if TRUE
    ...
    try:
        if second_pass == False:
            condition = ("obtain_representation" not in task) and (
                trial_to_run not in ["Linear", "Fine-Tuning"]
            )
        ...
        if (
            condition
        ):  # and trial_to_run not in ['Linear','Fine-Tuning']: #do not skip if you need to do finetuning
            ...
            if "train_val_metrics_dict" in os.listdir() and evaluation == False:
                if current_seed < max_seed - 1:
                    ...
                    save_path_dir = save_path_dir.replace(
                        "seed%i" % current_seed, seed_path
                    )
                    save_path_dir, seed = make_dir(
                        save_path_dir,
                        max_seed,
    ...
    if (
        os.path.isdir(save_path_dir) == False
    ):  # just in case we miss making the directory somewhere
        os.makedirs(save_path_dir)
