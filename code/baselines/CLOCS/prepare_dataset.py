...
class my_dataset_contrastive(Dataset):
    ...
    def __init__(
        ...
        if not (
            task == "contrastive_ss" and trial in ["Linear", "Fine-Tuning", "Random"]
        ):  #!= 'multi_task_learning':
            input_array, output_array, pid_array = self.load_raw_inputs_and_outputs(
                dataset_name, leads
            )
        ...
        fraction = fractions[
            "fraction"
        ]  # needs to be a list when dealing with 'query' or inference = True for CL scenario
        ...
        if task == "self-supervised":
            for modality in modalities:
                if phase == "train":
                    modality_input = np.concatenate(
                        list(input_array[modality][fraction][phase].values())
                    )
                    modality_output = np.concatenate(
                        list(output_array[modality][fraction][phase].values())
                    )
                else:
            ...
            self.label_array = [
                ...
                for _ in range(modality_input.shape[0])
            ]
            ...
            self.name = "-".join(
                (dataset_name, modalities[0], str(fraction), leads, class_pair)
            )  # name for different tasks
            if phase == "train":
                if inference == False:
                    inputs, outputs, pids = self.expand_labelled_data(
                        input_array,
                        output_array,
                        pid_array,
                        fraction,
                    ...
                    (
                        ...
                        modality_array,
                        keep_indices,
                    ) = self.retrieve_modified_unlabelled_data(
                        input_array,
                        output_array,
                        fraction,
                        unlabelled_fraction,
                ...
                inputs, outputs, pids = self.retrieve_val_data(
                    input_array,
                    output_array,
                    pid_array,
                    phase,
    ...
    def load_raw_inputs_and_outputs(self, dataset_name, leads="i"):
        ...
        if dataset_name == "bidmc":
            ...
            lead = ["II", "aVR"]
            path = os.path.join(
                basepath, "cipa-ecg-validation-study-1.0.0", "leads_%s" % lead
            )
            extension = ""
        ...
        with open(
            os.path.join(path, "frames_phases_%s%s.pkl" % (extension, dataset_name)),
            "rb",
        ) as f:
            print(
                os.path.join(
                    path, "frames_phases_%s%s.pkl" % (extension, dataset_name)
                ),
                "rb",
            )
        ...
        with open(
            os.path.join(path, "labels_phases_%s%s.pkl" % (extension, dataset_name)),
            "rb",
        ) as g:
            output_array = pickle.load(g)
        """ Dict Containing Patient Numbers """
        with open(
            os.path.join(path, "pid_phases_%s%s.pkl" % (extension, dataset_name)), "rb"
        ) as h:
            pid_array = pickle.load(h)  # needed for CPPC (ours)
        return input_array, output_array, pid_array

    def offset_outputs(
        self, dataset_name, outputs, t=0
    ):  # t tells you which class pair you are on now (used rarely and only for MTL)
    ...
    def retrieve_buffered_data(self, buffer_indices_dict, fraction, labelled_fraction):
        ...
        for task_name, indices in buffer_indices_dict.items():
            ...
            if self.cl_scenario == "Class-IL":
                self.class_pair = "-".join(
                    task_name.split("-")[-2:]
                )  # b/c e.g. '0-1' you need last two
            elif self.cl_scenario == "Time-IL":
                self.class_pair = task_name.split("-")[-1]
            elif (
                self.cl_scenario == "Task-IL" and "chapman" in dataset
            ):  # chapman ecg as task in Task-IL setting
                self.class_pair = task_name.split("-")[-1]
            input_array, output_array = self.load_raw_inputs_and_outputs(dataset, leads)
            input_array, output_array = self.retrieve_labelled_data(
                input_array,
                output_array,
                fraction,
                labelled_fraction,
            ...
            task_indices_buffer.append(
                indices
            )  # will go 1-10K, 1-10K, etc. not cumulative indices
    ...
    def expand_labelled_data_with_buffer(
        self,
        input_array,
        output_array,
        buffer_indices_dict,
    ...
    def retrieve_val_data(
        ...
        inputs, outputs, pids, _ = self.shrink_data(
            inputs, outputs, pids, labelled_fraction
        )
    ...
    def shrink_data(self, inputs, outputs, pids, fraction, modality_array=None):
        ...
        random.seed(
            0
        )  # to make sure we always obtain SAME shrunken dataset for reproducibility
    ...
    def remove_acquired_data(self, inputs, outputs, modality_array, acquired_indices):
        keep_indices = list(
            set(list(np.arange(inputs.shape[0]))) - set(acquired_indices)
        )
    ...
    def retrieve_unlabelled_data(
        self, input_array, output_array, fraction, unlabelled_fraction
    ):  # ,modalities=['ecg','ppg']):
        phase = "train"
        frame_array = []
        label_array = []
        ...
        inputs, outputs, modality_array = self.shrink_data(
            inputs, outputs, unlabelled_fraction, modality_array
        )

        return inputs, outputs, modality_array

    ### This is function you want for MC Dropout Phase ###
    def retrieve_modified_unlabelled_data(
        self, input_array, output_array, fraction, unlabelled_fraction, acquired_indices
    ):
        inputs, outputs, modality_array = self.retrieve_unlabelled_data(
            input_array, output_array, fraction, unlabelled_fraction
        )
        inputs, outputs, modality_array, keep_indices = self.remove_acquired_data(
            inputs, outputs, modality_array, acquired_indices
        )
        return inputs, outputs, modality_array, keep_indices
    ...
    def retrieve_labelled_data(
        ...
        if (
            self.cl_scenario == "Class-IL"
            or self.cl_scenario == "Time-IL"
            or dataset_name
            in [
                "chapman",
                "emg",
    ...
    def acquire_unlabelled_samples(
        self, inputs, outputs, fraction, unlabelled_fraction, acquired_indices
    ):
        inputs, outputs, modality_array = self.retrieve_unlabelled_data(
            inputs, outputs, fraction, unlabelled_fraction
        )
        if len(acquired_indices) > 1:
            ...
            modality_array = np.array(
                list(itemgetter(*acquired_indices)(modality_array))
            )
            ...
            modality_array = np.expand_dims(
                np.array(modality_array[acquired_indices[0]]), 1
            )
        return inputs, outputs, modality_array
    ...
    def retrieve_multi_task_train_data(self):
        ...
        for t, (dataset_name, fraction, leads, class_pair) in enumerate(
            zip(self.dataset_name, self.fraction, self.leads, all_class_pairs)
        ):  # should be an iterable list
            current_input, current_output, pid = self.load_raw_inputs_and_outputs(
                dataset_name, leads=leads
            )
            self.class_pair = class_pair
            self.modalities = all_modalities[
                t
            ]  # list(map(lambda x:x[0],all_modalities))
            # print(self.labelled_fraction)
            current_input, current_output, pid = self.retrieve_labelled_data(
                current_input,
                current_output,
                pid,
                fraction,
                self.labelled_fraction,
                dataset_name=dataset_name,
            )
    ...
    def retrieve_multi_task_val_data(self, phase):
        ...
        for t, (dataset_name, modalities, fraction, leads, class_pair) in enumerate(
            ...
            self.class_pair = class_pair
            self.modalities = modalities
            current_input, current_output, pid = self.retrieve_val_data(
                current_input,
                current_output,
                pid,
                phase,
    ...
    def expand_labelled_data(
        ...
        inputs, outputs, pids = self.retrieve_labelled_data(
            input_array,
            output_array,
            pid_array,
            fraction,
        ...
        if len(acquired_indices) > 0:
            (
                ...
                acquired_outputs,
                acquired_modalities,
            ) = self.acquire_unlabelled_samples(
                input_array,
                output_array,
                fraction,
                unlabelled_fraction,
    ...
    def normalize_frame(self, frame):
        if self.dataset_name not in [
            ...
            "physionet2017",
            "physionet2016",
        ]:  # or self.dataset_name != 'physionet2017':# or self.dataset_name != 'cipa':
    ...
    def __getitem__(self, index):
        true_index = self.remaining_indices[
            index
        ]  # this should represent indices in original unlabelled set
        ...
        nsamples = input_frame.shape[
            0
        ]  # (5000,) for my approach, #2500 for CMC approach (OURS1)
        if self.trial == "CMSC":
            ...
            frame_views = torch.empty(
                1, window_len, self.nviews
            )  # self.nviews) #nviews = nleads in this case (1x2500x12)
            start = 0
            ...
            self.trial == "CMSMLC"
        ):  # contrastive multi-segment multi-lead coding (OURS3)
            frame = torch.tensor(input_frame, dtype=torch.float)
            label = torch.tensor(label, dtype=torch.float)
            frame = frame.unsqueeze(0)  # (1,5000,12) #SxL = Samples x Leads
            frame_views = torch.empty(
                1, window_len, self.nviews * 2
            )  # self.nviews) #nviews = nleads in this case (1x2500x12*nsegments)
            ...
            label = torch.tensor(label, dtype=torch.float)
            frame = frame.unsqueeze(0)  # (1,5000)
            frame_views = frame.unsqueeze(
                2
            )  # to show that there is only 1 view (1x2500x1)
