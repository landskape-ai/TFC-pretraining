...
from prepare_miscellaneous import (calculate_auc, change_labels_type,
                                   flatten_arrays, obtain_contrastive_loss)
...
def contrastive_single(
    ...
    weighted_sampling,
    epoch_count=None,
    new_task_epochs=None,
    trial=None,
    save_path_dir=None,
):  # b/c it is single, models_list contains one model only
    ...
    for inputs, labels, pids, modality, task_names, indices in tqdm(dataloaders[phase]):
        ...
        with torch.set_grad_enabled(
            "train1" in phase
        ):  # and inference == False): #('train' in phase and inference == False)
            outputs = model(
                inputs
            )  # (BxHx2) in CPPC, (BxHx12) in CMLC, (BxHx24) in CMSMLC
...
def one_epoch_contrastive(
    ...
    (
        ...
        new_task_epochs=new_task_epochs,
        trial=trial,
...
def finetuning_single(
    ...
    print(
        f"acc {acc}, precision {precision}, recall {recall}, f1 {f1}, auroc {auroc}, auprc {auprc}"
    )
    epoch_loss = running_loss / len(dataloaders[phase].dataset)
...
def one_epoch_finetuning(
    ...
    return (
        {"epoch_loss": epoch_loss, "epoch_auroc": epoch_auroc},
        outputs_list,
        labels_list,
        modality_list,
        indices_list,
