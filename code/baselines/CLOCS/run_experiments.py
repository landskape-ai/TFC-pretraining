...
from prepare_miscellaneous import (
    determine_classification_setting, make_saving_directory_contrastive,
    modify_dataset_order_for_multi_task_learning, obtain_information,
    obtain_load_path_dir, obtain_saved_weights_name)
...
if nleads == 12:
    leads_list = [
        ...
        "['I']",
        "['I']",
    ]  #'II' for one lead, ['II','V1',etc.] for more leads
elif nleads == 4:
    leads_list = [
        None,
        None,
...
downstream_task_options = [
    ...
    "contrastive_ml",
    "contrastive_msml",
    "obtain_representation_contrastive",
]  # load normal data, load patient data for CPPC, load patient data for CPPC
""" ------------ """
...
trials_to_load_dict = {
    ...
    },  # needs to be a list to allow for sequence of perturbations
    "SimCLR": {
        "input_perturbed": True,  # default is perturbed - do not change
        "perturbation": ["Gaussian"],
    },
...
trials_to_run_dict = {
    ...
    "Linear": {
        ...
        "nviews": 1,
    },  # changing this will require remaking dataset
    "Fine-Tuning": {
        "downstream_task": "contrastive_ss",  # load ordinary datasets
        "nencoders": 1,  # this will depend on original self-supervision method used e.g. CPPC, CMC, etc.
        "nviews": 1,
...
def run_configurations(
    ...
    for trial_to_load, trial_to_run in zip(trial_to_load_list, trial_to_run_list):
        for (
            embedding_dim
        ) in embedding_dim_list:  # embedding dimension to use for pretraining
            for (
                downstream_dataset
            ) in downstream_dataset_list:  # dataset used for pretraining
                for (
                    second_dataset
                ) in second_dataset_list:  # dataset used for evaluation down the line
                    for labelled_fraction in labelled_fraction_list:
                        downstream_task, nencoders, nviews = trials_to_run_dict[
                            trial_to_run
                        ].values()
                        input_perturbed, perturbation = trials_to_load_dict[
                            trial_to_load
                        ].values()
                        ...
                        for seed in seeds:
                            seed = 4
                            save_path_dir, seed = make_saving_directory_contrastive(
                                phases,
                                downstream_dataset,
                                trial_to_load,
                                trial_to_run,
                            ...
                            if trial_to_run in ["Linear", "Fine-Tuning", "Random"]:
                                (
                                    original_downstream_dataset,
                                    modalities,
                                    leads,
                                    class_pair,
                                    fraction,
                                ) = modify_dataset_order_for_multi_task_learning(
                                    second_dataset,
                                    modalities,
                                    leads,
                                    class_pair,
                            ...
                            load_path_dir, save_path_dir = obtain_load_path_dir(
                                phases,
                                save_path_dir,
                                trial_to_run,
                                second_dataset,
                            ...
                            classification = determine_classification_setting(
                                second_dataset, trial_to_run
                            )
                            train_model(
                                basepath_to_data,
...
trial_to_load_list = [
    "CMSC"
]  # for loading pretrained weights, dropped SimCLR which is baseline
trial_to_run_list = [
    "Fine-Tuning"
]  # ['Linear','Linear','Linear','Linear'] #['Fine-Tuning','Fine-Tuning','Fine-Tuning','Fine-Tuning'] #['Linear','Linear','Linear','Linear'] #['Fine-Tuning','Fine-Tuning','Fine-Tuning','Fine-Tuning'] #['Fine-Tuning','Fine-Tuning','Fine-Tuning','Fine-Tuning']  #['Linear','Linear','Linear','Linear']  #['Random']#,'Fine-Tuning','Fine-Tuning','Fine-Tuning']#['SimCLR','CMSC','CMLC','CMSMLC'] #current trial to run and perform training # Fine-Tuning | Same as trial_to_load
embedding_dim_list = [320]  # 320,256,128,64,32
downstream_dataset_list = [
    "sleepEDF"
]  # ,'physionet2020'] #dataset for pretraininng # 'chapman' | 'physionet2020'
second_dataset_list = [
    "pFD_B"
...
labelled_fraction_list = [
    1
]  # 0.25,0.50,0.75,1.00] #proportion of labelled training data to train on # SHOULD BE 1 for pretraining #[0.25,0.50,0.75,1.00] for finetuning and linear evaluation
