...
class FCN(nn.Module):
    def __init__(self, n_in):
        ...
        self.proj_head = nn.Sequential(
            nn.Linear(128, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Linear(128, 128)
        )
...
def train_mixup_model_epoch(model, training_set, test_set, optimizer, alpha, epochs):
    ...
    training_generator = DataLoader(
        training_set, batch_size=batch_size_tr, shuffle=True, drop_last=True
    )
...
def test_model(model, training_set, test_set):
    ...
    N_te = len(test_set.x)
    print(N_te)
    test_generator = DataLoader(test_set, batch_size=1, shuffle=True, drop_last=False)
    ...
    metrics_dict["Precision"] = sklearn.metrics.precision_score(
    ...
    metrics_dict["AUROC"] = sklearn.metrics.roc_auc_score(
        target_prob, pred_prob, multi_class="ovr"
    )
    metrics_dict["AUPRC"] = sklearn.metrics.average_precision_score(
...
model = th.load(
    os.path.join(basepath, pretrained, "model" + str(seed)),
    map_location=th.device("cuda"),
)
...
LossListM, AccListM = train_mixup_model_epoch(
    model, training_set, test_set, optimizer, alpha, epochs
)
