...
def jitter(x, window_len, sigma=0.8):
    ...
    for i in range(
        d1
    ):  # add noise to a segment of each sample at independently random locations
        for j in range(d2):
            begin = np.random.randint(0, last_time_idx)
            xtilde[i, j, begin : begin + sub_len] += noise_mag * np.random.normal(
                loc=0.0, scale=sigma, size=(1, 1, sub_len)
            )
    return xtilde
