...
class TC(nn.Module):
    ...
    def forward(self, features_aug1, features_aug2):
        ...
        t_samples = (
            torch.randint(seq_len - self.timestep, size=(1,)).long().to(self.device)
        )  # randomly pick time stamps

        nce = 0  # average over timestep and batch
        encode_samples = (
            torch.empty((self.timestep, batch, self.num_channels))
            .float()
            .to(self.device)
        )

        for i in np.arange(1, self.timestep + 1):
            encode_samples[i - 1] = z_aug2[:, t_samples + i, :].view(
...
class convEncoder(nn.Module):
    def __init__(self, configs, device, kernel_sizes, num_channels):
        ...
        self.device = device
        self.conv_Q_encoders = nn.ModuleList(
            [
                nn.Conv1d(num_channels, self.feature_len, kernel_size=n, padding="same")
                for n in self.kernel_sizes
            ]
        )
        self.conv_V_encoders = nn.ModuleList(
            [
                nn.Conv1d(num_channels, self.feature_len, kernel_size=n, padding="same")
                for n in self.kernel_sizes
            ]
        )
        self.conv_K_encoders = nn.ModuleList(
            [
                nn.Conv1d(num_channels, self.feature_len, kernel_size=n, padding="same")
                for n in self.kernel_sizes
            ]
    ...
    def forward(self, X):
        heads_out = []
        for Qe, Ve, Ke in zip(
            self.conv_Q_encoders, self.conv_V_encoders, self.conv_K_encoders
        ):
            Q = Qe(X)
            V = Ve(X)
            K = Ke(X)
            score = (
                torch.bmm(Q.transpose(1, 2), K) / self.dim
            )  #             K, Q, V of shape batch_size (nb) * feature_len (fl) * window size/time steps (ts)
            attn = F.softmax(
                score, -1
            )  #             Q.T = nb * ts * fl ; K = nb * fl * ts, score = nb * ts * ts
            context = torch.bmm(attn, V.transpose(1, 2)).transpose(
                1, 2
            )  # nb * fl * ts, same as QVK
...
class TS_SD(nn.Module):
    def __init__(self, configs, device, kernel_sizes, num_channels, num_encoders):
        ...
        self.conv_encoders = nn.ModuleList(
            [
                convEncoder(configs, device, kernel_sizes, num_channels)
